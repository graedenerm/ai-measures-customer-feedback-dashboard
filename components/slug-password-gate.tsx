...
interface SlugPasswordGateProps {
  slug: string
  password: string
  companyName: string
...
export function SlugPasswordGate({ slug, password, companyName, children }: SlugPasswordGateProps) {
  const storageKey = `auth_${slug}`
  const [authenticated, setAuthenticated] = useState<boolean | null>(null)
  const [input, setInput] = useState('')
  const [error, setError] = useState(false)

  useEffect(() => {
    setAuthenticated(localStorage.getItem(storageKey) === '1')
  }, [storageKey])
